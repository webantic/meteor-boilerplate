if (Meteor.isServer) {
  ...
  Doodles.allow({
	...
	Meteor.methods({
		'custom-method-url-prefix': function() {
			return 'foo';
		}
	});

	Meteor.method('custom-method-url-prefix-single', function () {
		return 'foo';
	});

	Meteor.publish('custom-publish-url-prefix', function () {
		this.added('test', {foo: 'bar'});
		this.ready();
	});
